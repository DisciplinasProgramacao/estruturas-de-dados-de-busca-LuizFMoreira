...
public class ABB<K, V> implements IMapeamento<K, V>{
    ...
    //Tarefa 1: metodo recortar
    public Lista<V> recortar(K chaveDeOnde, K chaveAteOnde) {
    Lista<V> resultado = new Lista<>();
    recortarAux(raiz, chaveDeOnde, chaveAteOnde, resultado);
    return resultado;
}

private void recortarAux(No<K, V> no, K de, K ate, Lista<V> lista) {
    if (no == null) return;

    // Comparações usando o comparador da árvore
    int compInicio = comparador.compare(no.getChave(), de);
    int compFim = comparador.compare(no.getChave(), ate);

    // Ainda pode haver elementos >= de na subárvore esquerda
    if (compInicio >= 0) {
        recortarAux(no.getEsquerda(), de, ate, lista);
    }

    // Se a chave está dentro do intervalo inclusivo
    if (compInicio >= 0 && compFim <= 0) {
        lista.inserir(no.getItem());

    }

    // Ainda pode haver elementos <= ate na subárvore direita
    if (compFim <= 0) {
        recortarAux(no.getDireita(), de, ate, lista);
    }
}
